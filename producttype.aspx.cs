...
public partial class producttype : BaseForm
{
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!Page.IsPostBack)
        {
           ...
           // deleteBtn.Attributes.Add("OnClick", "return  del_sure()");
            ...
            List<ProductType> plt = GetJsonToObject<List<ProductType>>(jpath);
            myRepeat.DataSource = null;
            myRepeat.DataSource = plt;
            myRepeat.DataBind();
        }
    }
    //删除
    protected void deleteBtn_Click(object sender, EventArgs e)
    {
        ...
        //删除指定
        string jpath = @"\json\Producttype.json";
        List<ProductType> pls = GetJsonToObject<List<ProductType>>(jpath);
        int j = 0;
        ...
        //保存修改
        string s2 = JsonSerializer<List<ProductType>>(pls);
